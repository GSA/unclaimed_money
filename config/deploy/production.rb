...
role :web, "172.22.73.89", "172.23.73.89"     # Your HTTP server, Apache/etc
role :app, "172.22.73.89", "172.23.73.89"     # This may be the same as your `Web` server
...
namespace :deploy do
  ...
  task :set_up_rvm, :roles => :app do
    run "cd #{release_path}; #{sudo} rvm use ruby-1.9.3-p484@unclaimed_money --create --rvmrc", :pty => true
    sudo "chown -R #{web_user} /usr/local/rvm/gems/ruby-1.9.3-p484@unclaimed_money", :pty => true
  end
end
